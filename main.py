...
# ─────────────────────────── Palette ──────────────────────────────────────────
BG        = "#1C1510"   # deep espresso
BG2       = "#251D17"   # card bg
BG3       = "#2F2520"   # sidebar / input bg
AMBER     = "#D4850A"   # primary accent
AMBER_LT  = "#F0A030"   # hover / bright
AMBER_DK  = "#8B5500"   # dark accent
TEXT      = "#EDE0CE"   # main text
TEXT_DIM  = "#8B7D6B"   # muted text
BORDER    = "#3D3028"   # subtle border
RED       = "#C0392B"
GREEN     = "#27AE60"
FONT      = ("Georgia", 11)
FONT_SM   = ("Georgia", 9)
FONT_LG   = ("Georgia", 14, "bold")
...
# ─────────────────────────── Reusable widgets ─────────────────────────────────
def styled_btn(parent, text, cmd, fg=BG, bg=AMBER, **kw):
    b = tk.Button(parent, text=text, command=cmd,
                  bg=bg, fg=fg, relief="flat", cursor="hand2",
                  font=FONT, activebackground=AMBER_LT, activeforeground=BG,
                  padx=14, pady=6, **kw)
    b.bind("<Enter>", lambda e: b.config(bg=AMBER_LT))
    b.bind("<Leave>", lambda e: b.config(bg=bg))
    return b
 
def styled_entry(parent, width=40, show=None):
    e = tk.Entry(parent, bg=BG3, fg=TEXT, insertbackground=AMBER,
...
def styled_text(parent, h=10, w=60):
    t = tk.Text(parent, bg=BG3, fg=TEXT, insertbackground=AMBER,
                relief="flat", font=FONT, height=h, width=w,
...
class AuthWindow(tk.Tk):
    ...
    def _build_setup(self):
        self._clear()
        fr = tk.Frame(self, bg=BG, padx=40, pady=30)
        fr.pack(expand=True)
 
        tk.Label(fr, text="📓", bg=BG, font=("Georgia", 42)).pack()
        tk.Label(fr, text="MY DIGITAL DIARY", bg=BG, fg=AMBER,
                 font=("Georgia", 16, "bold")).pack(pady=(0,4))
        tk.Label(fr, text="Create your password to begin", bg=BG,
                 fg=TEXT_DIM, font=FONT_SM).pack(pady=(0,20))
 
        for lbl, attr in [("New Password", "_p1"), ("Confirm Password", "_p2")]:
            tk.Label(fr, text=lbl, bg=BG, fg=TEXT_DIM, font=FONT_SM, anchor="w").pack(fill="x")
            e = styled_entry(fr, show="●")
            e.pack(pady=(2,10), ipady=4)
            setattr(self, attr, e)
 
        styled_btn(fr, "🔐  Create Password", self._do_setup).pack(pady=8, fill="x")
        tk.Label(fr, text="", bg=BG, fg=RED, font=FONT_SM).pack()
        self._msg = fr.winfo_children()[-1]
    ...
    def _build_login(self):
        ...
        tk.Label(fr, text="MY DIGITAL DIARY", bg=BG, fg=AMBER,
                 font=("Georgia", 16, "bold")).pack(pady=(0,4))
        tk.Label(fr, text="Enter your password", bg=BG,
                 fg=TEXT_DIM, font=FONT_SM).pack(pady=(0,20))
 
        tk.Label(fr, text="Password", bg=BG, fg=TEXT_DIM, font=FONT_SM, anchor="w").pack(fill="x")
        self._pwd = styled_entry(fr, show="●")
        self._pwd.pack(pady=(2,12), ipady=4)
        self._pwd.bind("<Return>", lambda e: self._do_login())
        self._pwd.focus_set()
 
        styled_btn(fr, "🔑  Unlock Diary", self._do_login).pack(fill="x", pady=4)
        self._msg = tk.Label(fr, text="", bg=BG, fg=RED, font=FONT_SM)
    ...
    def _do_setup(self):
        ...
        if p1 != p2:
            ...
            return
        cfg = {"password_hash": hash_password(p1), "created_at": get_ts()}
        save_config(cfg)
    ...
    def _do_login(self):
        cfg = load_config()
        if hash_password(self._pwd.get()) == cfg.get("password_hash", ""):
            self._authenticated = True
            self.destroy()
        else:
            self.attempts -= 1
            if self.attempts:
                self._msg.config(text=f"Wrong password — {self.attempts} attempt(s) left.")
            else:
...
class DiaryApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(self.sidebar, text="DIGITAL DIARY", bg=BG3, fg=AMBER,
                 font=("Georgia", 11, "bold")).pack(pady=(0,20))
